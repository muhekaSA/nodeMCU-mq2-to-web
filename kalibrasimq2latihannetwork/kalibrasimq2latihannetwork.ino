...
const char* MY_SSID = "Sheilla";
const char* MY_PWD =  "Semakinjaya1";
WiFiClient client;
void setup()
{
  pinMode(led, OUTPUT);
...
void loop()
{
  int vmax = analogRead(MQ_PIN);
  int gas;
  int xco;
  int xasap;
  gas=MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_LPG); 
   xco=MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO); 
    xasap=MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_SMOKE);
 ...
 //DB Insert
  ...
  client.print("GET/mahasiswa/projectpermatavre/load.php?gas=");
  client.print(gas);
